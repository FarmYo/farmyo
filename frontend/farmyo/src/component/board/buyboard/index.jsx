...
export default function BuyBoardList({ value, search }) {
  ...
  const obsHandler = (entries) => {
    ...
    if (haveMore && target.isIntersecting && preventRef.current && boardInfo) {
      //옵저버 중복 실행 방지
  ...
  const getBoard = () => {
    api
      .get(`boards?type=1&page=${page}&size=${size}`)
      .then((res) => {
  ...
  return (
    <div style={{ height: "420px", position: "relative" }}>
      ...
      <Modal open={buyOpen} onClose={buyCloseModal} styles={styles}>
        ...
        <div className="mt-24">
          <label htmlFor="price" className="block text-md leading-6 text-gray-900">
            수량
          </label>
